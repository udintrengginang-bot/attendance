        ...
        <div class="mb-6">
            ...
            ${data.workers.map(w => `
                <div class="flex gap-3 items-center mb-3 p-3 border rounded-lg">
                    ...
                    <button type="button" data-worker-id="${w.id}" class="save-single-task px-3 py-1 bg-blue-500 text-white rounded text-sm">Save</button>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initApp() {
        ...
        document.getElementById('mobile-menu-btn')?.addEventListener('click', () => document.body.classList.toggle('sidebar-open'));
        ...
        [document.getElementById('dark-mode-toggle-mobile'), document.getElementById('dark-mode-toggle-desktop')].forEach(btn => {
            btn?.addEventListener('click', () => {
                ...
                document.querySelectorAll('#dark-mode-toggle-mobile span, #dark-mode-toggle-desktop span').forEach(icon => {
        ...
        const renderHome = async () => {
            ...
            activityList.innerHTML = recent.length ? recent.map(log => {
                ...
                return `<div class="flex gap-3 p-3 hover:bg-slate-50 rounded-lg"><span class="text-2xl">${icon}</span><div><p class="font-semibold">${log.laborerName} ${log.action.toLowerCase()} at ${site?.name || 'Unknown'}</p><p class="text-sm text-slate-500">${time}</p></div></div>`;
        ...
        const renderWorkers = () => {
            ...
            page.innerHTML = `<div class="flex justify-between items-center mb-6">
                ...
                <button data-action="add-worker" class="bg-amber-500 hover:bg-amber-600 font-bold py-3 px-6 rounded-lg">Add Worker</button>
            </div><div class="bg-white rounded-xl shadow-lg overflow-x-auto"><table class="w-full"><thead><tr class="border-b-2"><th class="py-3 px-4 text-left">Name</th><th class="py-3 px-4 text-left">Mobile</th><th class="py-3 px-4 text-left">Status</th><th class="py-3 px-4 text-right">Actions</th></tr></thead><tbody>${laborersData.map(l => `<tr class="border-b hover:bg-slate-50"><td class="py-4 px-4 font-medium">${l.name}</td><td class="py-4 px-4">${l.mobileNumber}</td><td class="py-4 px-4"><span class="px-2 py-1 text-xs rounded-full ${l.status === 'Work Started' ? 'bg-green-100 text-green-800' : 'bg-slate-100'}">${l.status || 'Work Ended'}</span></td><td class="py-4 px-4 text-right"><button data-action="edit-worker" data-id="${l.id}" class="text-blue-600 font-semibold mr-2">Edit</button><button data-action="delete-worker" data-id="${l.id}" class="text-red-600 font-semibold">Delete</button></td></tr>`).join('')}</tbody></table></div>`;
        };
        ...
        const renderSites = () => {
            ...
            page.innerHTML = `<div class="flex justify-between items-center mb-6">
                ...
                <button data-action="add-site" class="bg-amber-500 hover:bg-amber-600 font-bold py-3 px-6 rounded-lg">Add Site</button>
            </div><div class="bg-white rounded-xl shadow-lg overflow-x-auto"><table class="w-full"><thead><tr class="border-b-2"><th class="py-3 px-4 text-left">Site Name</th><th class="py-3 px-4 text-left">Location</th><th class="py-3 px-4 text-right">Actions</th></tr></thead><tbody>${sitesData.map(s => `<tr class="border-b hover:bg-slate-50"><td class="py-4 px-4 font-medium">${s.name}</td><td class="py-4 px-4">${s.location}</td><td class="py-4 px-4 text-right"><button data-action="edit-site" data-id="${s.id}" class="text-blue-600 font-semibold mr-2">Edit</button><button data-action="delete-site" data-id="${s.id}" class="text-red-600 font-semibold">Delete</button></td></tr>`).join('')}</tbody></table></div>`;
        };
        ...
        const renderTasks = () => {
            ...
            page.innerHTML = `<div class="flex justify-between items-center mb-6">
                ...
                <button data-action="assign-tasks" class="bg-amber-500 hover:bg-amber-600 font-bold py-3 px-6 rounded-lg">Assign Tasks</button>
        ...
        const renderSummary = async () => {
            ...
            page.innerHTML = `<h2 class="text-3xl font-bold mb-6">Project Summary</h2><div class="bg-white rounded-xl shadow-lg overflow-x-auto"><table class="w-full"><thead><tr class="border-b-2"><th class="py-3 px-4 text-left">Site</th><th class="py-3 px-4 text-right">Labor Cost</th><th class="py-3 px-4 text-right">Expenses</th><th class="py-3 px-4 text-right">Total</th></tr></thead><tbody>${Object.values(costs).map(c => `<tr class="border-b hover:bg-slate-50"><td class="py-4 px-4 font-medium">${c.name}</td><td class="py-4 px-4 text-right">${currencyFormatter.format(c.labor)}</td><td class="py-4 px-4 text-right">${currencyFormatter.format(c.expenses)}</td><td class="py-4 px-4 text-right font-bold text-amber-600">${currencyFormatter.format(c.labor + c.expenses)}</td></tr>`).join('')}<tr class="bg-slate-100 font-bold"><td colspan="3" class="py-4 px-4 text-right">GRAND TOTAL:</td><td class="py-4 px-4 text-right text-amber-600">${currencyFormatter.format(total)}</td></tr></tbody></table></div>`;
        };
        ...
        const renderExpenses = () => {
            ...
            page.innerHTML = `<div class="flex justify-between items-center mb-6">
                ...
                <button data-action="add-expense" class="bg-amber-500 hover:bg-amber-600 font-bold py-3 px-6 rounded-lg">Add Expense</button>
            </div><div class="bg-white rounded-xl shadow-lg overflow-x-auto"><table class="w-full"><thead><tr class="border-b-2"><th class="py-3 px-4 text-left">Date</th><th class="py-3 px-4 text-left">Site</th><th class="py-3 px-4 text-left">Description</th><th class="py-3 px-4 text-right">Amount</th><th class="py-3 px-4 text-right">Actions</th></tr></thead><tbody>${sorted.map(e => `<tr class="border-b hover:bg-slate-50"><td class="py-4 px-4">${e.date}</td><td class="py-4 px-4">${sitesData.find(s => s.id === e.siteId)?.name || 'N/A'}</td><td class="py-4 px-4">${e.description}</td><td class="py-4 px-4 text-right">${currencyFormatter.format(e.amount)}</td><td class="py-4 px-4 text-right"><button data-action="edit-expense" data-id="${e.id}" class="text-blue-600 font-semibold mr-2">Edit</button><button data-action="delete-expense" data-id="${e.id}" class="text-red-600 font-semibold">Delete</button></td></tr>`).join('')}</tbody></table></div>`;
        };
        ...
        const renderAttendance = () => {
            ...
            page.innerHTML = `<h2 class="text-3xl font-bold mb-6">Attendance Log</h2><div class="bg-white rounded-xl shadow-lg overflow-x-auto"><table class="w-full"><thead><tr class="border-b-2"><th class="py-3 px-4 text-left">Date & Time</th><th class="py-3 px-4 text-left">Worker</th><th class="py-3 px-4 text-left">Site</th><th class="py-3 px-4 text-left">Action</th></tr></thead><tbody>${sorted.map(log => {
                ...
                return `<tr class="border-b hover:bg-slate-50"><td class="py-4 px-4">${date || 'N/A'}</td><td class="py-4 px-4 font-medium">${log.laborerName}</td><td class="py-4 px-4">${site?.name || 'Unknown'}</td><td class="py-4 px-4 ${color} font-semibold">${log.action}</td></tr>`;
        ...
        const renderGallery = async () => {
            ...
            try {
                ...
                page.innerHTML = `<h2 class="text-3xl font-bold mb-6">📸 Work Photo Gallery</h2><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">${photos.length ? photos.map(p => {
                    ...
                    return `<div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        const routes = {
            '#home': renderHome,
            '#dashboard': async () => document.getElementById('dashboard').innerHTML = '<h2 class="text-3xl font-bold mb-6">Dashboard Statistics Coming Soon</h2>',
